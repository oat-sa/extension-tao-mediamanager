function _typeof(obj){
    ...
    return function(container,config){
        var areaBroker,itemCreator,pluginLoader=pluginLoaderFactory(),passageAuthoring=componentFactory({getItemCreator:function getItemCreator(){
            ...
            var $container=this.getElement();areaBroker=areaBrokerFactory($container,{menu:$container.find(".menu"),menuLeft:$container.find(".menu-left"),menuRight:$container.find(".menu-right"),contentCreatorPanel:$container.find("#item-editor-panel"),editorBar:$container.find("#item-editor-panel .item-editor-bar"),title:$container.find("#item-editor-panel .item-editor-bar h1"),toolbar:$container.find("#item-editor-panel .item-editor-bar #toolbar-top"),interactionPanel:$container.find("#item-editor-interaction-bar"),propertyPanel:$container.find("#item-editor-item-widget-bar"),itemPanel:$container.find("#item-editor-scroll-inner"),itemPropertyPanel:$container.find("#sidebar-right-item-properties"),itemStylePanel:$container.find("#item-style-editor-bar"),modalContainer:$container.find("#modal-container"),elementPropertyPanel:$container.find("#item-editor-body-element-property-bar .panel")});
        }).on("destroy",function(){
            itemCreator&&itemCreator.destroy(),itemCreator=null,areaBroker=null;
        });return _.defer(function(){
            return passageAuthoring.init(config);
        }),passageAuthoring;
    };
}),define("taoMediaManager/controller/manageMedia",["jquery","i18n","module","helpers","layout/actions/binder","uri","ui/previewer","layout/section","taoMediaManager/qtiCreator/component/passageAuthoring","core/request"],function($,__,module,helpers,binder,uri,previewer,section,passageAuthoringFactory,request){
    return{start:function start(){
        var $previewer=$(".previewer"),file={};file.url=$previewer.data("url"),file.mime=$previewer.data("type"),$previewer.data("xml")?$.ajax({url:file.url,data:{xml:!0},method:"POST"}).success(function(response){
            file.xml=response,$previewer.previewer(file);
        }):$previewer.previewer(file),$("#edit-media").off().on("click",function(){
            var action={binding:"load",url:helpers._url("editMedia","MediaImport","taoMediaManager")};binder.exec(action,{classUri:$(this).data("classuri"),id:$(this).data("uri")}||this._resourceContext);
        }),binder.register("newPassage",function(actionContext){
            var self=this,classUri=uri.decode(actionContext.classUri),signature=actionContext.signature;return"class"!==actionContext.type&&(signature=actionContext.classSignature),request({url:self.url,method:"POST",data:{id:classUri,type:"instance",signature:signature},dataType:"json"}).then(function(response){
                if(response.success&&response.data)return actionContext.tree&&$(actionContext.tree).trigger("addnode.taotree",[{uri:uri.decode(response.data.uri),label:response.data.label,parent:uri.decode(actionContext.classUri),cssClass:"node-instance"}]),{uri:uri.decode(response.data.uri),label:response.data.label,classUri:uri.decode(actionContext.classUri),type:"instance"};throw new Error(__("Adding the new resource has failed"));
            });
        }),binder.register("passageAuthoring",function(actionContext){
            var data=_.pick(actionContext,["id"]);section.create({id:"authoring",name:__("Authoring"),url:this.url,content:"",visible:!1}).show();passageAuthoringFactory($("#panel-authoring"),{plugins:[{module:"taoMediaManager/qtiCreator/plugins/content/title",bundle:"taoMediaManager/loader/taoMediaManager.min",category:"content"}],properties:{uri:actionContext.uri,label:"Asset",baseUrl:"/",itemDataUrl:`http://bosa/taoMediaManager/MediaManager/getFile?uri=${encodeURIComponent(uri.decode(actionContext.uri))}`}});
        });
    }};
}),define("taoMediaManager/controller/routes",[],function(){
    return{MediaManager:{actions:{editInstance:"controller/manageMedia"}}};
}),define("taoMediaManager/controller/taskView",["jquery","lodash","i18n","module","core/logger","ui/feedback","layout/loading-bar","util/url"],function($,_,__,module,loggerFactory,feedback,loadingBar,urlHelper){
    var defaults={returnUrl:urlHelper.route("index","TaoMediaManager","taoMediaManager")};return{start:function start(){
        var options=_.defaults({},appController.options,module.config(),defaults);loadingBar.start();try{
            taskActionFactory(appController.content.getElement(),options).on("viewcreate",function(){
                return loadingBar.start();
            }).on("viewrender",function(name,component){
                return component.on("success",function(message){
                    return feedback().success(message);
                });
            }).on("ready viewrender",function(){
                return loadingBar.stop();
            }).on("submit",function(){
                loadingBar.start(),this.close();
            }).on("close",function(){
                return appController.redirect(options.returnUrl);
            }).on("error",appController.onError);
        }catch(err){
            appController.onError(err);
        }
    }};
}),define("taoMediaManager/qtiCreator/plugins/content/title",["jquery","lodash","i18n","core/plugin"],function($,_,__,pluginFactory){
    return pluginFactory({name:"title",init:function(){
        var config=this.getHost().getConfig(),item=this.getHost().getItem();item&&!_.isEmpty(item.attr("title"))?this.title=item.attr("title"):config&&config.properties&&config.properties.label&&(this.title=config.properties.label);
    },render:function(){
        this.title&&this.getAreaBroker().getTitleArea().text(this.title);
    }});
}),define("tpl!taoMediaManager/qtiCreator/plugins/button",["handlebars"],function(hb){
    return hb.template(function(Handlebars,depth0,helpers,partials,data){
        this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",escapeExpression=this.escapeExpression;return buffer+="<li class=\"btn-info small ",(helper=helpers.cssClass)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.cssClass,stack1="function"===_typeof(helper)?helper.call(depth0,{hash:{},data:data}):helper),buffer+=`${escapeExpression(stack1)}" title="`,(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,stack1="function"===_typeof(helper)?helper.call(depth0,{hash:{},data:data}):helper),buffer+=`${escapeExpression(stack1)}">\n    <span class="li-inner"><span class="icon-`,(helper=helpers.icon)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.icon,stack1="function"===_typeof(helper)?helper.call(depth0,{hash:{},data:data}):helper),buffer+=`${escapeExpression(stack1)}"></span>`,(helper=helpers.text)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.text,stack1="function"===_typeof(helper)?helper.call(depth0,{hash:{},data:data}):helper),buffer+=`${escapeExpression(stack1)}</span>\n</li>`,buffer;
    });
}),define("taoMediaManager/qtiCreator/plugins/menu/save",["jquery","i18n","core/plugin","ui/hider","tpl!taoMediaManager/qtiCreator/plugins/button"],function($,__,pluginFactory,hider,buttonTpl){
    return pluginFactory({name:"save",init:function(){
        var self=this,itemCreator=this.getHost();this.$element=$(buttonTpl({icon:"save",title:__("Save the asset"),text:__("Save"),cssClass:"save-trigger"})).on("click",function(e){
            e.preventDefault(),self.disable(),itemCreator.trigger("save");
        }),this.hide(),this.disable(),itemCreator.on("ready saved error",function(){
            self.enable();
        });
    },render:function(){
        var $container=this.getAreaBroker().getMenuArea();$container.append(this.$element),this.show();
    },destroy:function(){
        this.$element.remove();
    },enable:function(){
        this.$element.removeProp("disabled").removeClass("disabled");
    },disable:function(){
        this.$element.prop("disabled",!0).addClass("disabled");
    },show:function(){
        hider.show(this.$element);
    },hide:function(){
        hider.hide(this.$element);
    }});
}),define("taoMediaManager/qtiCreator/plugins/menu/preview",["jquery","i18n","core/plugin","ui/hider","taoItems/previewer/factory","tpl!taoMediaManager/qtiCreator/plugins/button"],function($,__,pluginFactory,hider,previewerFactory,buttonTpl){
    return pluginFactory({name:"preview",init:function(){
        var self=this,itemCreator=this.getHost();itemCreator.on("preview",function(uri){
            previewerFactory("qtiItem",uri,{},{readOnly:!1,fullPage:!0});
        }),this.$element=$(buttonTpl({icon:"preview",title:__("Preview the asset"),text:__("Preview"),cssClass:"preview-trigger"})).on("click",function(e){
            $(document).trigger("open-preview.qti-item"),e.preventDefault(),self.disable(),itemCreator.trigger("preview",itemCreator.getItem().data("uri")),self.enable();
        });
    },render:function(){
        var $container=this.getAreaBroker().getMenuArea();$container.append(this.$element);
    },destroy:function(){
        this.$element.remove();
    },enable:function(){
        this.$element.removeProp("disabled").removeClass("disabled");
...
}),define("taoMediaManager/qtiCreator/plugins/menu/print",["jquery","i18n","core/plugin","ui/hider","tpl!taoMediaManager/qtiCreator/plugins/button"],function($,__,pluginFactory,hider,buttonTpl){
    var spreadClass=function($container,cssClass){
        $container.parents().each(function(){
            $(this).siblings().addClass(cssClass);
        });
    };return pluginFactory({name:"print",init:function(){
        var self=this,itemCreator=this.getHost();spreadClass(this.getAreaBroker().getItemPanelArea(),"item-no-print"),this.$element=$(buttonTpl({icon:"print",title:__("Print the asset"),text:__("Print"),cssClass:"print-trigger"})).on("click",function(e){
            e.preventDefault(),window.print();
        }),this.hide(),this.disable(),itemCreator.on("ready",function(){
            self.enable();
        });
    },render:function(){
        var $container=this.getAreaBroker().getMenuArea();$container.append(this.$element),this.show();
...
}),define("taoMediaManager/qtiCreator/plugins/loader",["core/pluginLoader","taoMediaManager/qtiCreator/plugins/menu/save","taoMediaManager/qtiCreator/plugins/menu/preview","taoMediaManager/qtiCreator/plugins/menu/print","taoMediaManager/qtiCreator/plugins/content/title"],function(pluginLoader,save,preview,print,title){
    return pluginLoader({menu:[save,preview,print],content:[title],panel:[]});
}),define("taoMediaManager/qtiCreator/plugins/navigation/back",["jquery","i18n","core/plugin","ui/hider","tpl!taoMediaManager/qtiCreator/plugins/button"],function($,__,pluginFactory,hider,buttonTpl){
    return pluginFactory({name:"back",init:function(){
        var itemCreator=this.getHost();itemCreator.on("exit",function(){
            window.history.back();
        }),this.$element=$(buttonTpl({icon:"left",title:__("Back to Manage Assets"),text:__("Manage Assets"),cssClass:"back-action"})).on("click",function(e){
            e.preventDefault(),itemCreator.trigger("exit");
        }),this.hide();
    },render:function(){
        var $container=this.getAreaBroker().getMenuLeftArea();$container.append(this.$element),this.show();
    },destroy:function(){
        this.$element.remove();
    },enable:function(){
        this.$element.removeProp("disabled").removeClass("disabled");
